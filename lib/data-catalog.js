...
const ID_FIELDS = new Set(['id', 'itemid', 'itemdata', 'itemguid', 'guid', 'key', 'assetid', 'definitionid', 'itemdefinitionid']);
const NAME_FIELDS = new Set(['name', 'itemname', 'displayname', 'label', 'title']);
const IMAGE_FIELDS = new Set(['image', 'imageurl', 'icon', 'iconpath', 'iconurl', 'thumbnail', 'sprite']);
...
function recordsFrom(value, source, category, output = [], inheritedId = null) {
  ...
  if (id != null && nameKey && ['string', 'number'].includes(typeof id)) {
    output.push({
      id: String(id), name: String(value[nameKey]),
      category: typeof value.category === 'string' ? value.category : category,
      dataset: category, source, imageHint: imageKey ? String(value[imageKey]) : null, data: value
    });
  }
